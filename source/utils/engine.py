...
class Trainer(object):
    ...
    def save(self, is_best=False):
        ...
        if is_best:
            ...
            shutil.move(model_file, best_model_file)
            shutil.move(train_file, best_train_file)
            self.logger.info(
            ...
            worse_model_file = os.path.join(self.save_dir, "worse.model")
            worse_train_file = os.path.join(self.save_dir, "worse.train")
            shutil.move(model_file, worse_model_file)
            shutil.move(train_file, worse_train_file)
            self.logger.info(
                "Saved worse model state to '{}' with new worse valid metric {}-{:.3f}".format(
                    worse_model_file, self.valid_metric_name.upper(), self.best_valid_metric))
